...
Práctica:                   PR0606. Limpieza de logs (II)
...
1. **Filtrado de seguridad:** ignora todos los registros que tengan el nivel `[LOG-OK]`. Solo procesa `ALERT` y `CRIT`.
...
3. **Normalización de fechas:** todas las fechas (sin importar si usan puntos, guiones o barras) deben transformarse al formato estándar **dd/MM/yyyy HH:mm**.
...
5. **Salida:** genera una lista de `[PSCustomObject]` y expórtala a un archivo llamado `reporte_inventario.csv`.
...
Severidad Item           Fecha            ActionRequired
--------- ----           -----            --------------
